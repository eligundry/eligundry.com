...
/**
 * Adds EXIF metadata to a JPEG image buffer.
 * Embeds alt text in the ImageDescription field.
 */
const addExifMetadata = async (
  imageBuffer: Buffer,
  altText: string,
  username: string
): Promise<Buffer> => {
    ...
    .withMetadata({
      exif: {
        IFD0: {
          ...
          Copyright: `Generated by eligundry.com for ${username}`,
...
/**
 * Gets the collage with EXIF metadata embedded.
 * Alt text is stored in the ImageDescription EXIF field.
 */
const getCollageWithExif = async (
  username: string,
  period: LastFMPeriod = '7day'
) => {
  const { collage, albums } = await getCollage(username, period)

  const altText = albums
    .slice(0, 9)
    .map(
  ...
  const collageWithExif = await addExifMetadata(
    collage,
    `Last.fm collage: ${altText}`,
    username
  )

  return {
    collage: collageWithExif,
    albums,
    altText,
  }
}
