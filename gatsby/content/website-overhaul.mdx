...
slug: changelog-2021-website-redesign
...
based content on a site like medium.com or dev.to will get more eyeballs on it than something you make on your own, but
there is something extremely satisfying in owning every pixel on a site with your name on it. I saw this Tweet the other
day and it seemed fitting to how I think about personal sites.
...
I this would be a technology blog if I didn't nerd out about tools before describing the cool things I did.

## Gatsby

![Gatsby Logo](./img/website-update/gatsby-logo.png)
...
learned there and ported them conceptually back to my personal site. I will make note of these learnings as I go.
...
after the JS has compiled, they are instead extracted from queries as a Babel transformation and transpiled to a JSON
...
<GitHubFileEmbed fileURL="https://github.com/eligundry/eligundry.com/blob/f9aa93ddf5e384e621ebd549b87e4e72defb00e0/gatsby/src/components/Daylio/useLatestFeelings.ts" />
...
3. The component uses a hosted instance [CORS Anywhere][cors-anywhere] on Heroku that would get rate limited frequently

I threw up my hands and realized that I needed to make a custom Gatsby source to pull this data at compile time. At
first, I had it living as a file inside my site, but then I started making custom Gatsby source packages at work and
...
It's very easy to pull a Last.fm collage of your top albums. A bunch of services exist for this. Why bring this up?
...
2. I'm overlaying the artist name, album name and the number of scrobbles with an image map
...
Sadly, this only works with Chrome + Edge, so even Safari & FireFox users don't get to see it 😭.

It's super dynamic, very cute and fits the theme of my site perfectly! I converted it into a React component with
a button in the bottom right part of the screen that can regenerate the pattern. It's even responsive to my dark mode
toggle, speaking of which...

<GitHubFileEmbed fileURL="https://github.com/eligundry/eligundry.com/blob/blog-bragging/gatsby/src/layout/FancyBackground.tsx" />
...
Unfortunately, Github does not provide this out of the box. Luckily, a service call [EmGithub][emgithub] provides
...
Fortunately, lots of ads work this way and some ad tech developers created a library called [postscribe][postscribe],
...
<GitHubFileEmbed fileURL="https://github.com/eligundry/eligundry.com/blob/master/gatsby/config/utils/addGitLastmodifiedToNode.ts" />
...
[amrit-nagi]: https://www.buymeacoffee.com/tailwindtoolbox
